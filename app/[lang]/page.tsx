...
export default function LanguagePage({ params }: PageProps) {
  ...
  // Example retrieval: https://developers.google.com/maps/documentation/places/web-service/place-id
  const GOOGLE_PLACE_ID = "ChIJgxFjPGE_HBURltZmCN5zmdA"
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-start py-6 sm:py-8">
        <div className="w-[min(420px,92vw)] bg-black/45 backdrop-blur-md border border-zinc-800 rounded-3xl shadow-2xl overflow-hidden">
          ...
          <div className="px-4 py-3">
            <div className="flex justify-center gap-3">
              <a href="https://www.facebook.com/profile.php?id=100005352488791" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-12 h-12 rounded-full bg-[#1877F2] text-white shadow-md hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 transition-all duration-150" aria-label="Facebook">
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
              </a>
              <a href="https://www.instagram.com/tattoobysashas?igsh=bGhnYTE0Z3pmZHF5" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-12 h-12 rounded-full bg-gradient-to-br from-purple-600 via-pink-600 to-orange-500 text-white shadow-md hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 transition-all duration-150" aria-label="Instagram">
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><rect x="2" y="2" width="20" height="20" rx="5" ry="5" fill="none" stroke="currentColor" strokeWidth="2"/><circle cx="12" cy="12" r="3" fill="currentColor"/><circle cx="18" cy="6" r="1" fill="currentColor"/></svg>
              </a>
              <a href="https://www.tiktok.com/@tattoobysashas?_r=1&_t=ZS-91JObky2tWx" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-12 h-12 rounded-full bg-zinc-900 text-white shadow-md hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 transition-all duration-150" aria-label="TikTok">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-5.2 1.74 2.89 2.89 0 012.31-4.64 2.93 2.93 0 01.88.13V9.4a6.84 6.84 0 00-1-.05A6.33 6.33 0 005 20.1a6.34 6.34 0 0010.86-4.43v-7a8.16 8.16 0 004.77 1.52v-3.4a4.85 4.85 0 01-1-.1z" /></svg>
              </a>
              <a href="https://wa.me/972526202701" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-12 h-12 rounded-full bg-[#25D366] text-white shadow-md hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 active:shadow-md transition-all duration-150" aria-label="WhatsApp">
        ...
        <div className="w-[min(420px,92vw)] mt-6 sm:mt-8">
          <div className="bg-black/45 backdrop-blur-md border border-zinc-800 rounded-3xl shadow-2xl overflow-hidden p-4">
            <h2 className="text-center font-bold text-xl sm:text-2xl mb-4 text-white tracking-wide">{t("portfolio")}</h2>
            <PortfolioCarousel />
          </div>
        </div>
        <div className="w-[min(420px,92vw)] mt-6 sm:mt-8">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex-shrink-0 hidden sm:block">
              <div className="relative w-[60px] h-[60px]">
                <Image src="/images/studio-logo.png" alt={`${t("title")} Logo`} width={60} height={60} className="drop-shadow-lg" style={{ clipPath: "circle(40% at 50% 50%)" }} />
              </div>
            </div>
            ...
            <div className={`text-center ${dir === 'rtl' ? 'sm:text-left' : 'sm:text-right'} flex-grow sm:flex-grow-0`}>
              <p className="text-zinc-500 text-xs m-0 mb-1">{t("copyright", { year: new Date().getFullYear() })}</p>
